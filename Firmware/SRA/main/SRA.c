...
static void circle_move_mm_blocking_R(int cx, int cy, int radius, int segments) {
    ...
    for (int i = 1; i <= segments/2; i++) { //clockwise right half
        angle = i * angle_step;
        int x = cx + (int)llround((double)radius * sin(angle));
        int y = cy + (int)llround((double)radius * cos(angle));
        line_move_mm_constant(x, y);
    }

}

static void circle_move_mm_blocking_S(int cx, int cy, int radius, int segments, int dir) {
    double angle_step = 2.0 * M_PI / (double)segments;
    double angle = 0.0;
...
void draw_S(int startx,int starty){
    line_move_mm_constant(25, 50);
    line_move_mm_constant(50, 100);
    vTaskDelay(pdMS_TO_TICKS(15000));
    int ox = startx;
    int oy = starty;
    line_move_mm_blocking(ox + 75*SCALE, oy + 0*SCALE);
    circle_move_mm_blocking_S(ox + 75*SCALE, oy + 50*SCALE, 50*SCALE, 3600, 0);
    line_move_mm_blocking(ox + 50*SCALE, oy + 100*SCALE);
    circle_move_mm_blocking_S(ox + 50*SCALE, oy + 150*SCALE, 50*SCALE, 3600, 1);
    line_move_mm_blocking(ox + 125*SCALE, oy + 200*SCALE); //225, 300
    line_move_mm_blocking(ox + 150*SCALE, oy + 200*SCALE);
}
void draw_A(void){
    int ox = current_x_mm;
    int oy = current_y_mm;
    line_move_mm_blocking(ox + 68*SCALE, oy + 200*SCALE);
    vTaskDelay(pdMS_TO_TICKS(200));
    line_move_mm_blocking(ox + 136*SCALE, oy + 0*SCALE);
    line_move_mm_blocking(ox + 102*SCALE, oy + 100*SCALE);
    line_move_mm_blocking(ox + 34*SCALE, oy + 100*SCALE);

}
void draw_R(void){
    int ox = current_x_mm;
    int oy = current_y_mm;
    line_move_mm_blocking(ox + 60*SCALE, oy + 0*SCALE);
    circle_move_mm_blocking_R(ox + 60*SCALE, oy - 50*SCALE, 50*SCALE, 3600);
    line_move_mm_blocking(ox + 0*SCALE, oy - 100*SCALE);
    vTaskDelay(pdMS_TO_TICKS(200));
    line_move_mm_blocking(ox + 0*SCALE, oy + 0*SCALE );
    vTaskDelay(pdMS_TO_TICKS(200));
    line_move_mm_blocking(ox + 0*SCALE, oy - 100*SCALE);
    vTaskDelay(pdMS_TO_TICKS(200));
    line_move_mm_blocking(ox + 0*SCALE, oy - 200*SCALE);
    vTaskDelay(pdMS_TO_TICKS(200));
    line_move_mm_blocking(ox + 0*SCALE, oy - 100*SCALE);
    vTaskDelay(pdMS_TO_TICKS(200));
    line_move_mm_blocking(ox + 60*SCALE, oy - 100*SCALE);
    line_move_mm_blocking(ox + 125*SCALE, oy - 200*SCALE); // 375, 100
    line_move_mm_blocking(ox + 150*SCALE, oy - 200*SCALE);
}
void app_main(void) {
    setup_stepper_pins();
    ...
    int originx=50;
    int originy=100;
    draw_S(originx,originy);
    draw_R();
    draw_A();

    // // A
    // line_move_mm_blocking(68, 200);
    // vTaskDelay(pdMS_TO_TICKS(200));
    // line_move_mm_blocking(136, 0);
    // line_move_mm_blocking(102,100);
    // line_move_mm_blocking(34, 100);

    // // R
    // line_move_mm_blocking(200, 200);
    // int ox = 200;
    // int oy = 200;
    // line_move_mm_blocking(ox + 60, oy + 0);
    // circle_move_mm_blocking_R(ox + 60, oy - 50, 50, 3600);
    // line_move_mm_blocking(ox + 0, oy - 100);
    // line_move_mm_blocking(ox + 0, oy + 0 );
    // line_move_mm_blocking(ox + 0, oy - 200);
    // line_move_mm_blocking(ox + 0, oy - 100);
    // line_move_mm_blocking(ox + 60, oy - 100);
    // line_move_mm_blocking(ox + 120, oy - 200);
    /*
    // S
    line_move_mm_blocking(100, 100);
    int ox = 100;
    int oy = 100;
    line_move_mm_blocking(ox + 75, oy + 0);
    circle_move_mm_blocking_S(ox + 75, oy + 50, 50, 3600, 0);
    line_move_mm_blocking(ox + 50, oy + 100);
    circle_move_mm_blocking_S(ox + 50, oy + 150, 50, 3600, 1);
    line_move_mm_blocking(ox + 125, oy + 200);
    */
    // line_move_mm_blocking(200, 200);
    // circle_move_mm_blocking_S(200, 200, 50, 3600, 1);
